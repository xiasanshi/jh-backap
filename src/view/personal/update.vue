<template>
    <div>
        ...
        <mt-field label="名称：" placeholder="请输入名称" :state="data.name ? 'success' : 'error'"
                  v-model="data.name"></mt-field>
        <mt-field label="联系电话：" placeholder="请输入联系方式" :state="data.mobile ? 'success' : 'error'"
                  :attr="{ maxlength: 11 }" type="tel" v-model="data.mobile"></mt-field>
        <mt-field label="地址：" placeholder="请输入店铺地址" :state="data.address ? 'success' : 'error'"
                  v-model="data.address"></mt-field>
        <mt-field label="配送费：" placeholder="请输入配送费" :state="data.shippingPrice ? 'success' : 'error'"
                  v-model="data.shippingPrice"></mt-field>
        <mt-field label="起送金额：" placeholder="请输入最低起送金额" :state="data.basePrice ? 'success' : 'error'"
                  v-model="data.basePrice"></mt-field>
        <!--<mt-field label="商品分类：">-->
        ...
        <div @click="openPicker1" style="">
            <mt-field label="开始：" :state="open_time ? 'success' : 'error'" type="datetime"
                      v-model="open_time"></mt-field>
        </div>
        <mt-datetime-picker
            ref="pickerend"
            type="time"
        ...
        <div @click="openPicker2">
            <mt-field label="结束：" :state="close_time ? 'success' : 'error'" type="datetime"
                      v-model="close_time"></mt-field>
        </div>
        <div>
            <!-- 内容 -->
            <div style="width: 100%;">
